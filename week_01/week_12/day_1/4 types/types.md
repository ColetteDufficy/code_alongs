...
> Create a Bear class with property name but no getter so that we can see the test fail
...
As it stands, we have quite a lot of repetition in our code. Let's tidy things up a bit by declaring that our `bear` variable is going to be of `Bear` type at the top of the file instead of at every test.
...
We're also being repetitive by creating a new bear at the start of each test. To save doing this we can add in a setup step using `@Before`. This signals to JUnit a piece of code to run before each test is run. We have done test constructors before, this is what they look like in Java.
